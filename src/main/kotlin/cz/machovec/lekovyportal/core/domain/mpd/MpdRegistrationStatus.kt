...
@Entity
@Table(name = "mpd_registration_status")
data class MpdRegistrationStatus(
    @Id
    ...
    @Column(name = "name")
    val name: String?
) : BaseMpdEntity<MpdRegistrationStatus>() {

    override fun getUniqueKey(): String = code

    override fun copyPreservingIdAndFirstSeen(from: MpdRegistrationStatus): MpdRegistrationStatus {
        return this.copy(
            id = from.id,
    ...
    override fun markMissing(since: LocalDate): MpdRegistrationStatus {
        return this.copy(missingSince = since)
    }

    override fun getBusinessAttributeChanges(other: MpdRegistrationStatus): List<AttributeChange<*>> {
