...
@Entity
@Table(name = "mpd_source")
data class MpdSource(
    @Id
    ...
    @Column(name = "name")
    val name: String?
) : BaseMpdEntity<MpdSource>() {

    override fun getUniqueKey(): String = code

    override fun copyPreservingIdAndFirstSeen(from: MpdSource): MpdSource {
        return this.copy(
            id = from.id,
    ...
    override fun markMissing(since: LocalDate): MpdSource {
        return this.copy(missingSince = since)
    }

    override fun getBusinessAttributeChanges(other: MpdSource): List<AttributeChange<*>> {
