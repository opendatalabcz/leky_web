...
@Entity
@Table(name = "mpd_measurement_unit")
data class MpdMeasurementUnit(
    @Id
    ...
    @Column(name = "name")
    val name: String?
) : BaseMpdEntity<MpdMeasurementUnit>() {

    override fun getUniqueKey(): String = code

    override fun copyPreservingIdAndFirstSeen(from: MpdMeasurementUnit): MpdMeasurementUnit {
        return this.copy(
            id = from.id,
    ...
    override fun markMissing(since: LocalDate): MpdMeasurementUnit {
        return this.copy(missingSince = since)
    }

    override fun getBusinessAttributeChanges(other: MpdMeasurementUnit): List<AttributeChange<*>> {
