...
class MpdSourceProcessor(
    ...
    fun importData(csvBytes: ByteArray, validFromOfNewDataset: LocalDate, validToOfNewDataset: LocalDate?) {
        ...
        val existingRecords = sourceRepository.findAllByCodeIn(newCodes)
        val updatedRecords = mutableListOf<MpdSource>()
        ...
        sourceRepository.saveAll(updatedRecords)
        logger.info { "Processed ${updatedRecords.size} updates for MpdSource." }
    }

    private fun parseLine(line: String, validFromOfNewDataset: LocalDate): MpdSource? {
        val cols = line.split(";")
