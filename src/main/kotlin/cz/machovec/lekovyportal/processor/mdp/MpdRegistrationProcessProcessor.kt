...
class MpdRegistrationProcessProcessor(
    ...
    fun importData(csvBytes: ByteArray, validFromOfNewDataset: LocalDate, validToOfNewDataset: LocalDate?) {
        ...
        val existingRecords = registrationProcessRepository.findAllByCodeIn(newCodes)
        val updatedRecords = mutableListOf<MpdRegistrationProcess>()
        ...
        registrationProcessRepository.saveAll(updatedRecords)
        logger.info { "Processed ${updatedRecords.size} updates for MpdRegistrationProcess." }
    }

    private fun parseLine(line: String, validFromOfNewDataset: LocalDate): MpdRegistrationProcess? {
        val cols = line.split(";")
